...
public class Server_6001 {
	...
	public Server_6001() throws IOException{
		...
		server = new ServerSocket(6001, 70000, addr);
	...
	void connectToPeer() throws UnknownHostException, IOException{
		Socket client = new Socket("0.0.0.0", 6000);
		...
		new Thread(){
			public void run(){
				try {coop();}catch(IOException e){e.printStackTrace();}
			}
		}.start();
		//		Names.add("prsvr");
	...
	boolean chk(String name) throws IOException{
		if(outToPeer == null) return false;
		outToPeer.writeBytes("chck"+name+'\n');
		String x = inFromPeer.readLine();
		System.out.println("pass");
		return x.trim().equals("y");
	}
	...
	private void coop() throws IOException{
		while(true){
			if(inFromPeer.ready()){
